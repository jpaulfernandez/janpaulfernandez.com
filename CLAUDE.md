...
- Palette is dark (Phase 20). `ink-*` is the ground, `paper-*` is the type, `wine-*` is the accent — the inverse of the Phase 9–19 meanings. **Phase 21 cut wine back to three places and no others: an inline link, a focus ring, and `KeyTakeaway`.** It is not for headline words (`.flare` is deleted), kicker dots, badges, list markers, filled buttons, or hover states. On the `#050505` ground, `wine-400` (#E0446F, 4.9:1) is the accent-type colour — links, hover tints, `KeyTakeaway` — after Phase 28 darkened it from the pinker `wine-300`, which is now the focus ring only; `wine-500`/`wine-600` are fills only. `paper-600` is non-text.
