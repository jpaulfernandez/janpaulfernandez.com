...
- **Client JS (amended in Phase 20, trimmed in Phase 21).** The site is no longer zero-JS: `src/scripts/motion.ts` (GSAP + ScrollTrigger, ~46KB gzip) loads on every page. That is the *only* sanctioned bundle. Everything else still climbs the YAGNI ladder — vanilla before a framework, CSS before JS, static HTML before an island. **Nothing else may import GSAP:** a page asks for motion with a `data-*` attribute and `motion.ts` decides what it means. The vocabulary is `data-reveal`, `data-reveal-delay`, `data-reveal-stagger`, `data-hero` and `data-brighten` (Phase 28 — a scroll-scrubbed brighten from a legible grey resting state; used once, on the homepage bio); SplitText, Lenis and the unused `data-parallax` were removed in Phase 21.
