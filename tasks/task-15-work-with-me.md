...
3. **Contact:** Formspree free-tier form (name, email, message) posting to Paul's email — form ID is blocked-on-Paul; build the form with a placeholder action + clear TODO, plus always-working `mailto:` link. Include honeypot field. Cal.com link only if Paul provides one (YAGNI otherwise).
4. Form UX: labels (not placeholder-as-label), required indicators, Formspree redirect to a simple `/thanks` page (noindex).
5. JSON-LD: `service()` per offering + `faqPage()` from FAQ entries + breadcrumbs.

## Definition of Done

- [ ] All 6 sections render; services and FAQ editable in /keystatic
- [ ] Home teaser now reads the services collection (T08 hardcoding gone)
- [ ] FAQ works keyboard-only, no JS shipped for it
- [ ] Form posts to Formspree (or documented placeholder pending Paul's email); mailto works regardless
- [ ] FAQPage + Service JSON-LD valid on validator.schema.org
- [ ] /thanks has noindex; build + check pass
